	...
	stage('Cxconsole') {
		...
		script {
			...
			dir(tempDir) {
				...
				sh './runCxConsole.sh Scan -v -ProjectName "CxServer/Empty1" -CxServer http://172.35.1.19 -cxuser admin -cxpassword KsHPJ8JCD5otOLixmMov_ -LocationType folder -LocationPath ${WORKSPACE} -preset "Checkmarx Default"'
