	...
	stage('Cxconsole') {
		...
		script {
			...
			dir(tempDir) {
				...
				sh './runCxConsole.sh Scan -v -ProjectName "CxServer/Empty1" -CxServer http://172.35.1.19 -cxuser admin -cxpassword 4g#Tj8H12 -LocationType folder -LocationPath ${WORKSPACE} -preset "Checkmarx Default"'
